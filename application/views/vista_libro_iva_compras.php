...
//modificar iva de la orden de compra
function modificarIVAordenCompra(idComprobante, distractor) 
	{
		var respuesta = confirm("Está seguro de querer MODIFICAR el IVA de la orden de compra con ID. " + idComprobante + "?")
		if(respuesta)//si la respuesta es afirmativa
			{
				window.location = "<?php echo base_url();?>micontrolador/VistaEditarIVA/"+ idComprobante + "/" + distractor + "/";
			}
	}
...
<div class="datagrid">
<form name = buscarCuenta method = "POST" action = "<?php echo base_url();?>micontrolador/VistaLibroIVAcompras/">
<li>
                    <label class="desc">Seleccione la fecha para filtrar y ver el libro IVA de compras</label>
                    <div>
						Mes: <select name ="mes">
...
<table  width="100%"  style="font-size:10px;font-family: Georgia, 'Times New Roman', serif;"  cellspacing="0" cellpadding="0" border="1">
<thead style="background: #e8edff;color: #039;">
  <tr style ="border:1px solid black;">
  <td colspan = "12" style="color: #039;">
...
<th colspan="7">Compras GRAVADAS</TH>
<tr>
	<th>Internas</th>
...
?>
  <td class ="celdaLibroIva"><?php echo $datosEcnontrados->correlativo;?></td>
   ...
   <td align = right></td>
   <td align = "center">
	<a href = "#" onClick ="modificarIVAordenCompra('<?php echo $datosEcnontrados->id_comprobante;?>', '<?php echo time();?>')">
				<img src = "<?php echo base_url();?>images/Notes.png" title = "Editar IVA"></a>
   </td>
...
?>
 <script>alert('La operación a sido guardada, se ha MODIFICADO valor del IVA de la orden de compra con el ID:  <?php echo $idComprobante;?> .')
 window.location = "<?php echo base_url();?>micontrolador/VistaLibroIVAcompras/";
 </script>
<?php
