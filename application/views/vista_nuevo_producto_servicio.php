...
<?php 
if($this->uri->segment(3,0) != 2)//si es 2 es porque habrio una popup y no deseo que muestre el sub-menu
	{
?>
<hr>
...
elseif($segmento == 1029)
{
   ...
   if($idDeDondeSeCreaElProductoServicio != 2)
	{
?>
<META HTTP-EQUIV=REFRESH CONTENT=3;URL="<?php echo base_url();?>micontrolador/VistaNuevoProductoServicioDesdeCompras/">    
...
<legend class = legend1>Nuevo Servicio</legend>
<div align = "center">
<b>Agregar Solo SERVICIOS</b>
</div>
    ...
    <table width="100%" cellpadding="0" cellspacing="0" border="0">
		...
		<tr>
		<!--campos hidden-->
				<input name="iddedondesecreaelproductoservicio" type="hidden" size =25  id="iddedondesecreaelproductoservicio" value = "<?php echo $this->uri->segment(3,0);?>" readonly maxlength="255"/><!--id de desde donde se esta creando el producto o servicio 1 bodega y 2 par el modulo de comrpas-->
			    
				<?php
				
				//NOTA ESTO YA NO LO OCUPO PORQUE PORQUE EN EL PROYECTO ORIGINAL
				//ESTA EL MANTENIMIENTO DE MATERIALES
				//AQUI EN COMPRAS SOLO SERÁ EL DE SERVICIOS
				/*
				  ...
				  if($desdeQueVistaSeCrea == 2)//es desde el modulo compras la categoria sera 2 agregar detalles de productos y de la vista de la edicion de los detalles de productos en la orden de compra
					...
					*/
				?>
				<input name="categoria" type="hidden" size =25  id="categoria" value = "2" readonly maxlength="255"/><!--la cotegoria de materiales 1 materiales de bodega y 2 producto o servicios de compras-->

				<!--fin campos hidden ocultos-->
			<td align = center>		
                <li>
				
                    <label class="desc">C&oacute;digo Producto/Servicio (solo n&uacute;meros):<span class ="req">*</span></label>
                    <div>
		 ...
		 <tr>
           ...
           <div align = center> 
			...
			<?php
				if($this->uri->segment(3,0) != 2)//si es distto a 2 es que no se abrio una popup
					{
			?>
			...
			<?php
					...
					{
			?>
				<button type="button" name="" value="" class="css3button" onClick="jasvscript:window.close();">Cancelar</button>
			<?php
					}
			?>
          </div>
