...
?>
 <td><?php echo $primerDetalleEncontrado->codigocuenta;?></td>
  ...
  <td>
   ...
   ?></td>
    <form action = "<?php echo base_url();?>micontrolador/AgregarCuentasAlaPartidaPruebas/<?php echo $idPartida = $this->uri->segment(3,0)."/".$this->uri->segment(4,0);?>" METHOD = POST>
  <input type = text name = interactiva size = 45>
  </form>
<?php
$sumaCargos = 0;
$sumaAbonos = 0;
...
?>
<form action = "<?php echo base_url();?>micontrolador/d/<?php echo $cuentaAlaPartidaEncontradas["codigo"];//eso para que uando le de enter se vala el código del producto correspondiente a cada caja de texto?>/<?php echo $idPartida = $this->uri->segment(3,0)."/".$this->uri->segment(4,0);?>" METHOD = POST>
 <tr> 
